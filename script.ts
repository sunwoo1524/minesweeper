...
function setCustomGame(): void {
    let x: number = Number(prompt("크기는 무제한이지만, 크면 클 수록 렉이 많이 걸릴 수 있습니다.\n최소 지뢰 개수는 1개이며 게임 크기와 같거나 클 수 없습니다.\n가로 크기:"));
    let y: number = Number(prompt("세로 크기"));
    let m: number = Number(prompt("지뢰 개수"));
